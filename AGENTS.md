...
- Prefer existing naming: `CamelCase` for types, `snake_case` for functions/vars in C++; constants/macros ALL_CAPS.
...
- Extended Dynamic State 1 (the original EXT set) is core as of Vulkan 1.3—assume those dynamic states are always available on our 1.4 baseline; only EDS2/EDS3 need extension/feature checks.
