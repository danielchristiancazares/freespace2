...
**Status**: **PARTIALLY FIXED** - Null crash is fixed, but timing issue remains.

## Problem

`std::bad_function_call` in `stars_setup_environment_mapping` when calling `gr_screen.gf_calculate_irrmap()`.
...
- Implementation:
  ...
  - For each cubemap face:
    ...
    - `VulkanRenderer::beginAuxiliaryRenderPass(...)` with that face's `VulkanFramebuffer`
      - **Note**: `beginAuxiliaryRenderPass()` returns early (does nothing) if a scene/direct pass is active, but `gr_vulkan_calculate_irrmap()` doesn't check for this failure
