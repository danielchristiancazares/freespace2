...
public:
  ...
  template <typename F>
  explicit MoveOnlyFunction(F &&f) : m_fn(std::make_unique<Model<std::decay_t<F>>>(std::forward<F>(f))) {}
...
public:
  ...
  template <typename F> void enqueue(uint64_t retireSerial, F &&releaseFn) {
    Entry e;
