...
public:
  ...
  vk::DescriptorImageInfo getTextureDescriptor(const FrameCtx &ctx, int bitmapHandle,
  ...
  void setModelUniformBinding(VulkanFrame &frame, gr_buffer_handle handle, size_t offset, size_t size);
  void setSceneUniformBinding(VulkanFrame &frame, gr_buffer_handle handle, size_t offset, size_t size);
  ...
  void applySetupFrameDynamicState(const FrameCtx &ctx, const vk::Viewport &viewport, const vk::Rect2D &scissor,
                                   float lineWidth);
  ...
  void deferredLightingFinish(graphics::vulkan::RecordingFrame &rec, DeferredLightingCtx &&lighting,
                              const vk::Rect2D &restoreScissor);
  uint32_t getMinUniformBufferAlignment() const {
  ...
  MovieTextureHandle createMovieTexture(uint32_t width, uint32_t height, MovieColorSpace colorspace,
                                        MovieColorRange range);
  void uploadMovieTexture(const FrameCtx &ctx, MovieTextureHandle handle, const ubyte *y, int yStride, const ubyte *u,
                          int uStride, const ubyte *v, int vStride);
  void drawMovieTexture(const FrameCtx &ctx, MovieTextureHandle handle, float x1, float y1, float x2, float y2,
                        float alpha);
  ...
  vk::DescriptorSet allocateModelDescriptorSet() { return m_descriptorLayouts->allocateModelDescriptorSet(); }
...
private:
  ...
  void recordBloomBlurPass(const RenderCtx &render, VulkanFrame &frame, uint32_t srcPingPongIndex,
                           uint32_t variantFlags, int mipLevel, uint32_t bloomWidth, uint32_t bloomHeight);
  ...
  void recordSmaaBlendWeightsPass(const RenderCtx &render, VulkanFrame &frame,
                                  const vk::DescriptorImageInfo &edgesInput, const vk::DescriptorImageInfo &areaTex,
                                  const vk::DescriptorImageInfo &searchTex);
  void recordSmaaNeighborhoodPass(const RenderCtx &render, VulkanFrame &frame,
                                  const vk::DescriptorImageInfo &colorInput, const vk::DescriptorImageInfo &blendTex);
  ...
  void recordLightshaftsPass(const RenderCtx &render, VulkanFrame &frame,
                             const graphics::generic_data::lightshaft_data &params);
  void recordPostEffectsPass(const RenderCtx &render, VulkanFrame &frame,
                             const graphics::generic_data::post_data &params, const vk::DescriptorImageInfo &ldrInput,
