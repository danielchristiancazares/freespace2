...
constexpr ShaderLayoutSpec makeSpec(shader_type type,
	const char* name,
	PipelineLayoutKind layout,
...
constexpr std::array<ShaderLayoutSpec, NUM_SHADER_TYPES> buildSpecs()
{
	...
	return {
		...
		makeSpec(SDR_TYPE_POST_PROCESS_BLOOM_COMP, "SDR_TYPE_POST_PROCESS_BLOOM_COMP", PL::Standard, VI::VertexAttributes),
		makeSpec(SDR_TYPE_POST_PROCESS_BRIGHTPASS, "SDR_TYPE_POST_PROCESS_BRIGHTPASS", PL::Standard, VI::VertexAttributes),
		...
		makeSpec(SDR_TYPE_POST_PROCESS_SMAA_BLENDING_WEIGHT, "SDR_TYPE_POST_PROCESS_SMAA_BLENDING_WEIGHT", PL::Standard, VI::VertexAttributes),
...
{
	return getShaderLayoutSpec(type).pipelineLayout;
}
