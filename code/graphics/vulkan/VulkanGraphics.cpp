...
void gr_vulkan_delete_buffer(gr_buffer_handle handle) { currentRenderer().deleteBuffer(handle); }
...
void gr_vulkan_restore_screen(int id) {
  ...
  if (id >= 0) {
    Assertion(id == handle, "gr_vulkan_restore_screen called with handle %d but saved screen handle is %d", id, handle);
  }
...
void gr_vulkan_resize_buffer(gr_buffer_handle handle, size_t size) { currentRenderer().resizeBuffer(handle, size); }
...
void gr_vulkan_update_transform_buffer(void *data, size_t size) {
  ...
  const auto minAlign =
      static_cast<vk::DeviceSize>(ctxBase.renderer.vulkanDevice()->properties().limits.minStorageBufferOffsetAlignment);
  ...
  Assertion(alloc.offset <= std::numeric_limits<uint32_t>::max(), "Transform buffer offset %zu exceeds uint32 range",
            static_cast<size_t>(alloc.offset));
...
MovieTextureHandle gr_vulkan_movie_texture_create(uint32_t width, uint32_t height, MovieColorSpace colorspace,
...
void gr_vulkan_movie_texture_upload(MovieTextureHandle handle, const ubyte *y, int y_stride, const ubyte *u,
                                    int u_stride, const ubyte *v, int v_stride) {
  if (g_backend == nullptr || g_backend->renderer == nullptr || !g_backend->recording.has_value()) {
    return;
  }
  auto ctxBase = currentFrameCtx();
...
void gr_vulkan_movie_texture_draw(MovieTextureHandle handle, float x1, float y1, float x2, float y2, float alpha) {
  if (g_backend == nullptr || g_backend->renderer == nullptr || !g_backend->recording.has_value()) {
    return;
  }
  auto ctxBase = currentFrameCtx();
...
void stub_render_decals(decal_material * /*material_info*/, primitive_type /*prim_type*/, vertex_layout * /*layout*/,
                        int /*num_elements*/, const indexed_vertex_source & /*buffers*/,
                        const gr_buffer_handle & /*instance_buffer*/, int /*num_instances*/) {}

void stub_render_shield_impact(shield_material * /*material_info*/, primitive_type /*prim_type*/,
...
static void draw_decal_pass(const DecalBoundFrame &bound, const ShaderModules &modules,
                            const DecalTextureBindings &textures, gr_alpha_blend blendMode, const bvec4 &colorMask,
                            const indexed_vertex_source &source, gr_buffer_handle instanceBufferHandle,
                            const vertex_layout &layout, int numElements, int numInstances, uint32_t gbufferIndex) {
...
static void gr_vulkan_render_decals(decal_material *material_info, primitive_type prim_type, vertex_layout *layout,
                                    int num_elements, const indexed_vertex_source &buffers,
                                    const gr_buffer_handle &instance_buffer, int num_instances) {
  ...
  const ShaderModules emissiveModules =
      renderer.getShaderModulesByFilenames("decal.vert.spv", "decal_emissive.frag.spv");
  ...
  draw_decal_pass(bound, diffuseModules, textures, material_info->get_blend_mode(0), mask, buffers, instance_buffer,
                  *layout, num_elements, num_instances, 0);

  // Normal into gbuffer[1] (additive)
  draw_decal_pass(bound, normalModules, textures, material_info->get_blend_mode(1), mask, buffers, instance_buffer,
                  *layout, num_elements, num_instances, 1);

  // Emissive into gbuffer[4]
  draw_decal_pass(bound, emissiveModules, textures, material_info->get_blend_mode(2), mask, buffers, instance_buffer,
                  *layout, num_elements, num_instances, 4);
}
...
static void issueModelDraw(const RenderCtx &render, const ModelDrawContext &ctx) {
  ...
  cmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, ctx.pipelineLayout, 0, 1, &modelSet,
                         static_cast<uint32_t>(dynamicOffsets.size()), dynamicOffsets.data());

  // Push constants (vertex layout + texture indices)
  cmd.pushConstants(ctx.pipelineLayout, vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment, 0,
                    sizeof(ModelPushConstants), &ctx.pcs);
  ...
  const vk::IndexType indexType = use32BitIndices ? vk::IndexType::eUint32 : vk::IndexType::eUint16;

  // Index data is laid out at:
  //   vertSource.Index_offset (heap base) + batch.index_offset (per-batch byte offset)
  const vk::DeviceSize indexOffsetBytes = static_cast<vk::DeviceSize>(ctx.vertSource.Index_offset + batch.index_offset);
...
void gr_vulkan_render_model(model_material *material_info, indexed_vertex_source *vert_source, vertex_buffer *bufferp,
                            size_t texi) {
  ...
  // Base byte offset in the vertex heap for THIS vertex_buffer
  {
    ...
    Assertion(byteOffset <= std::numeric_limits<uint32_t>::max(), "Model vertex heap offset exceeds uint32 range");
  ...
  if (ctxBase.renderer.supportsExtendedDynamicState3()) {
    ...
    Assertion(rt.colorAttachmentCount <= VulkanRenderTargets::kGBufferCount,
              "render_model: unexpected colorAttachmentCount=%u (max=%u)", rt.colorAttachmentCount,
  ...
  ModelDrawContext ctx{
      bound, pipeline, layout, pcs, *vert_source, *bufferp, texi,
  };

  issueModelDraw(renderCtx, ctx);
}

void gr_vulkan_render_shield_impact(shield_material *material_info, primitive_type prim_type, vertex_layout *layout,
                                    gr_buffer_handle buffer_handle, int n_verts) {
  // Preconditions - frame injected by setup_frame, parameters must be valid
  ...
  vk::DescriptorImageInfo shieldMapInfo =
      (textureHandle >= 0) ? ctxBase.renderer.getTextureDescriptor(ctxBase, textureHandle, samplerKey)
...
void gr_vulkan_render_primitives(material *material_info, primitive_type prim_type, vertex_layout *layout, int offset,
                                 int n_verts, gr_buffer_handle buffer_handle, size_t buffer_offset) {
  // Preconditions - frame injected by setup_frame, parameters must be valid
  ...
  if (shaderType == SDR_TYPE_DEFAULT_MATERIAL) {
    ...
    for (size_t i = 0; i < layout->get_num_vertex_components(); ++i) {
      ...
      if (fmt == vertex_format_data::COLOR3 || fmt == vertex_format_data::COLOR4 ||
  ...
  Assertion(shaderModules.vert != nullptr, "render_primitives missing vertex shader for shaderType=%d",
            static_cast<int>(shaderType));
  Assertion(shaderModules.frag != nullptr, "render_primitives missing fragment shader for shaderType=%d",
            static_cast<int>(shaderType));
  ...
  // Get vertex buffer
  Assertion(buffer_handle.isValid(), "render_primitives called with invalid buffer handle (shaderType=%d, material=%p)",
  ...
  if (shaderType == SDR_TYPE_FLAT_COLOR) {
    ...
    if (iface_debug_count < 5) {
      mprintf(("Interface #%d: color=(%.2f,%.2f,%.2f,%.2f) intensity=%.2f tex=%d noTex=%d\n", iface_debug_count,
               clr.xyzw.x, clr.xyzw.y, clr.xyzw.z, clr.xyzw.w, intensity, textureHandle, noTexturing));
      ++iface_debug_count;
    }
  ...
  auto samplerKey = VulkanTextureManager::SamplerKey{};
  samplerKey.address = convertTextureAddressing(material_info->get_texture_addressing());
  baseMapInfo = isTextured ? ctxBase.renderer.getTextureDescriptor(ctxBase, textureHandle, samplerKey)
                           : ctxBase.renderer.getDefaultTextureDescriptor(samplerKey);
  ...
  // Push descriptors
  cmd.pushDescriptorSetKHR(vk::PipelineBindPoint::eGraphics, ctxBase.renderer.getPipelineLayout(),
                           0, // set 0
  ...
  gr_zbuffer_type zbufferMode = material_info->get_depth_mode();
  bool depthTest =
      (zbufferMode == gr_zbuffer_type::ZBUFFER_TYPE_READ || zbufferMode == gr_zbuffer_type::ZBUFFER_TYPE_FULL);
  bool depthWrite =
      (zbufferMode == gr_zbuffer_type::ZBUFFER_TYPE_WRITE || zbufferMode == gr_zbuffer_type::ZBUFFER_TYPE_FULL);
  const bool hasDepthAttachment = (rt.depthFormat != vk::Format::eUndefined);
  if (!hasDepthAttachment) {
    depthTest = false;
    depthWrite = false;
...
void gr_vulkan_render_nanovg(nanovg_material *material_info, primitive_type prim_type, vertex_layout *layout,
  ...
  pipelineKey.front_depth_fail_op =
      static_cast<VkStencilOp>(convertStencilOperation(frontStencilOp.depthFailOperation));
  ...
  Assertion(uniformBuffer, "Failed to resolve Vulkan uniform buffer for NanoVGData handle %d",
            nv.nanovgUbo.handle.value());
...
void gr_vulkan_render_primitives_batched(batched_bitmap_material *material_info, primitive_type prim_type,
  ...
  vk::DescriptorImageInfo baseMapInfo = ctxBase.renderer.getTextureDescriptor(ctxBase, textureHandle, samplerKey);
...
void gr_vulkan_render_rocket_primitives(interface_material *material_info, primitive_type prim_type,
                                        vertex_layout *layout, int n_indices, gr_buffer_handle vertex_buffer,
  ...
  cmd.bindPipeline(vk::PipelineBindPoint::eGraphics, pipeline);
  cmd.pushDescriptorSetKHR(vk::PipelineBindPoint::eGraphics, ctxBase.renderer.getPipelineLayout(), 0, writeCount,
                           writes.data());
...
void init_function_pointers() {
  ...
  // Depth/cull state
  gr_screen.gf_set_cull = [](int cull) -> int { return currentRenderer().setCullMode(cull); };
  gr_screen.gf_zbuffer_set = [](int mode) -> int { return currentRenderer().setZbufferMode(mode); };
  gr_screen.gf_zbuffer_get = []() -> int { return currentRenderer().getZbufferMode(); };
  gr_screen.gf_zbuffer_clear = [](int mode) { currentRenderer().zbufferClear(mode); };
  ...
  gr_screen.gf_map_buffer = [](gr_buffer_handle handle) -> void * { return currentRenderer().mapBuffer(handle); };
  ...
  gr_screen.gf_render_primitives_particle = [](particle_material *, primitive_type, vertex_layout *, int, int,
                                               gr_buffer_handle) {};
  gr_screen.gf_render_primitives_distortion = [](distortion_material *, primitive_type, vertex_layout *, int, int,
                                                 gr_buffer_handle) {};
...
VulkanRenderer *getRendererInstance() { return g_backend ? g_backend->renderer.get() : nullptr; }
