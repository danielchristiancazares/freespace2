...
void pushLightDescriptors(vk::CommandBuffer cmd, vk::PipelineLayout layout,
                          vk::Buffer buffer, uint32_t matrixOffset, uint32_t lightOffset) {
    ...
    uint32_t uniformAlignment)
{
    ...
    for (const auto& src : Lights) {
        ...
        if (src.type == Light_Type::Directional) {
            ...
            // Normalize
            float len = sqrtf(viewConeDir.xyzw.x * viewConeDir.xyzw.x +
                              viewConeDir.xyzw.y * viewConeDir.xyzw.y +
            ...
            float radius = (Lighting_mode == lighting_mode::COCKPIT)
                ? lp->cockpit_light_radius_modifier.handle(src.radb)
                : src.radb;
