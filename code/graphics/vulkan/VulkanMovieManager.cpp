...
void copyPlanePacked(ubyte* dst, uint32_t dstStride, const ubyte* src, int srcStride,
  uint32_t copyWidth, uint32_t copyHeight)
{
  ...
  for (uint32_t y = 0; y < copyHeight; ++y) {
	std::memcpy(dst + static_cast<size_t>(dstStride) * y,
	  src + static_cast<size_t>(srcStride) * y,
	  copyWidth);
  }
}
...
VulkanMovieManager::VulkanMovieManager(VulkanDevice& device, VulkanShaderManager& shaders)
  : m_vulkanDevice(device),
	...
	m_memoryProperties(device.memoryProperties()),
	m_pipelineCache(device.pipelineCache()),
...
void VulkanMovieManager::createMovieYcbcrConfigs()
{
  for (uint32_t i = 0; i < MOVIE_YCBCR_CONFIG_COUNT; ++i) {
	...
	convInfo.components = { vk::ComponentSwizzle::eIdentity,
...
MovieTextureHandle VulkanMovieManager::createMovieTexture(uint32_t width,
  uint32_t height,
  ...
  MovieColorRange range)
{
  ...
  try {
	...
	allocInfo.memoryTypeIndex = findMemoryType(req2.memoryRequirements.memoryTypeBits,
...
void VulkanMovieManager::uploadMovieFrame(const UploadCtx& ctx,
  ...
  int vStride)
{
  ...
  cmd.copyBufferToImage(ctx.frame.stagingBuffer().buffer(),
	tex.image.get(),
...
void VulkanMovieManager::drawMovieTexture(const RenderCtx& ctx,
  MovieTextureHandle handle,
  float x1,
  float y1,
  float x2,
  ...
  float alpha)
{
  ...
  if (ctx.targetInfo.colorFormat != m_swapchainFormat && !m_loggedTargetFormatMismatch) {
	vkprintf("VulkanMovieManager: movie draw target format (%d) does not match swapchain format (%d).\n",
	  static_cast<int>(ctx.targetInfo.colorFormat),
  ...
  cmd.pushConstants(cfg.pipelineLayout.get(),
