// Vulkan-local copy of lighting helpers.
// Kept isolated from the OpenGL .sdr pipeline under code/def_files/.
...
{
	return mix(specColor, FresnelSchlick(specColor, light, halfVec), fresnel) * ((specPower + 2.0) / 8.0 ) *
	       pow(clamp(dot(normal, halfVec), 0.0, 1.0), specPower) * dotNL;
}
